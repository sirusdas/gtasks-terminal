#!/usr/bin/env python3
"""
Final deduplication script that runs until no more duplicates are found
"""
...
def remove_duplicates():
    """Remove duplicates and return the number of duplicates removed"""
    print("🗑️ Scanning for duplicate tasks...")
...
def main():
    """Main function that runs deduplication until no more duplicates are found"""
    print("🔄 Starting continuous deduplication process...")
    ...
    while iteration <= max_iterations:
        print(f"\n🔁 Iteration {iteration}")
        duplicates_found = remove_duplicates()
        
        if duplicates_found == 0:
            print("\n✅ No more duplicates found. Deduplication process completed.")
            break
        elif duplicates_found < 0:
            print("\n❌ Error occurred during deduplication. Exiting.")
            return 1
        else:
            print(f"\n⚠️  Found {duplicates_found} duplicates. Waiting before next iteration...")
            if iteration < max_iterations:
                time.sleep(5)  # Wait 5 seconds before next iteration
        
        iteration += 1
    else:
        print(f"\n⚠️  Reached maximum iterations ({max_iterations}). Stopping deduplication process.")
